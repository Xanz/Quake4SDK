...
bool idActor::CanSeeFrom(const idVec3 &from, const idVec3 &toPos, bool useFov) const
{
	...
	if (tr.fraction >= 1.0f)
	{ // || ( gameLocal.GetTraceEntity( tr ) == ent ) ) {
		return true;
	}
	...
	for (ent = enemyList.Next(); ent != NULL; ent = ent->enemyNode.Next())
	{
		...
		if (distSquared < bestDistSquared)
		{
			if (checkPVS)
			{
				...
				if (pvs.i > 0 && pvs.i < MAX_CURRENT_PVS && !gameLocal.pvs.InCurrentPVS(pvs, ent->GetPVSAreas(), ent->GetNumPVSAreas()))
				{
					continue;
				}
			}

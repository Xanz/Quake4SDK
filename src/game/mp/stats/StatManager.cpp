...
{
	if (victim && attacker && (victim == attacker || gameLocal.IsTeamGame() && victim->IsType(idPlayer::GetClassType()) && attacker->IsType(idPlayer::GetClassType()) && static_cast<const idPlayer *>(victim)->team == static_cast<const idPlayer *>(attacker)->team))
	{
		return;
	}
...
void rvStatManager::GivePlayerCashForAward(idPlayer *player, inGameAward_t award)
{
	...
	{
	case IGA_CAPTURE:
		awardCashValueName = "playerCashAward_mpAward_capture";
		break;
	case IGA_HUMILIATION:
		awardCashValueName = "playerCashAward_mpAward_humiliation";
		break;
	case IGA_IMPRESSIVE:
		awardCashValueName = "playerCashAward_mpAward_impressive";
		break;
	case IGA_EXCELLENT:
		awardCashValueName = "playerCashAward_mpAward_excellent";
		break;
	case IGA_ASSIST:
		awardCashValueName = "playerCashAward_mpAward_assist";
		break;
	case IGA_DEFENSE:
		awardCashValueName = "playerCashAward_mpAward_defense";
		break;
	case IGA_COMBO_KILL:
		awardCashValueName = "playerCashAward_mpAward_combo_kill";
		break;
	case IGA_RAMPAGE:
		awardCashValueName = "playerCashAward_mpAward_rampage";
		break;
	case IGA_HOLY_SHIT:
		awardCashValueName = "playerCashAward_mpAward_holy_shit";
		break;
	}

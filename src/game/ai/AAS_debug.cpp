...
void idAASLocal::DrawAreaBounds(int areaNum) const
{
	...
	for (int p1 = 0; p1 < 8; p1++)
	{
		for (int p2 = 0; p2 < 8; p2++)
		{
			if (!drawn[p2][p1])
			{
				if ((points[p1].x == points[p2].x && (points[p1].y == points[p2].y || points[p1].z == points[p2].z)) || (points[p1].y == points[p2].y && (points[p1].x == points[p2].x || points[p1].z == points[p2].z)) || (points[p1].z == points[p2].z && (points[p1].x == points[p2].x || points[p1].y == points[p2].y)))
				{
					// an edge

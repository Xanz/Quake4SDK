...
bool rvMonsterGladiator::CheckActions(void)
{
	...
	if (usingShield)
	{
		...
		if (move.moveCommand == MOVE_TO_ENEMY && move.fl.moving)
		...
		if (PerformAction(&actionMeleeAttack, (checkAction_t)&idAI::CheckAction_MeleeAttack) ||
			...
			(railgunHealth > 0 && gameLocal.GetTime() - shieldStartTime > 2000 && gameLocal.time - pain.lastTakenTime > 500 && gameLocal.time - combat.shotAtTime > 300 && gameLocal.GetTime() - shieldLastHitTime > 500 && PerformAction(&actionRailgunAttack, (checkAction_t)&idAI::CheckAction_RangedAttack, &actionTimerSpecialAttack)))
		{
			shieldWaitTime = 0;
			return true;
		}
	...
	{ // Bring the shield out?
		...
		{ // not while rushing (NOTE: unless railgun was just destroyed?)
			if (enemy.fl.visible && enemy.fl.inFov)
			{
				...
				{
					if (gameLocal.time - pain.lastTakenTime <= 1500 || (combat.shotAtAngle < 0 && gameLocal.time - combat.shotAtTime < 100) || !gameLocal.random.RandomInt(20))
					{
...
{
	if (railgunHealth > 0)
	{ // Only let the gladiator rush when he is really close to his enemy
...
stateResult_t rvMonsterGladiator::State_Torso_ShieldStart(const stateParms_t &parms)
{
	...
	case STAGE_WAIT:
		if (AnimDone(ANIMCHANNEL_TORSO, 0) || animator.CurrentAnim(ANIMCHANNEL_TORSO)->GetEndTime() < 0 // anim somehow cycled?!!
			|| idStr::Icmp(animator.CurrentAnim(ANIMCHANNEL_TORSO)->AnimName(), "shield_end"))
		{ // anim changed
			SetShaderParm(6, 0);
...
stateResult_t rvMonsterGladiator::State_Torso_BlasterAttack(const stateParms_t &parms)
{
	...
	case STAGE_WAITSTART:
		if (AnimDone(ANIMCHANNEL_TORSO, 0) || animator.CurrentAnim(ANIMCHANNEL_TORSO)->GetEndTime() < 0 // anim somehow cycled?!!!
			|| idStr::Icmp(animator.CurrentAnim(ANIMCHANNEL_TORSO)->AnimName(), "blaster_start"))
		{ // anim changed
	...
	case STAGE_WAITLOOP:
		if (AnimDone(ANIMCHANNEL_TORSO, 0) || animator.CurrentAnim(ANIMCHANNEL_TORSO)->GetEndTime() < 0 // anim somehow cycled?!!!
			|| idStr::Icmp(animator.CurrentAnim(ANIMCHANNEL_TORSO)->AnimName(), "blaster_loop"))
		{ // anim changed
	...
	case STAGE_WAITEND:
		if (AnimDone(ANIMCHANNEL_TORSO, 4) || animator.CurrentAnim(ANIMCHANNEL_TORSO)->GetEndTime() < 0 // anim somehow cycled?!!!
			|| idStr::Icmp(animator.CurrentAnim(ANIMCHANNEL_TORSO)->AnimName(), "blaster_loop"))
		{ // anim changed
			return SRESULT_DONE;
		}
...
stateResult_t rvMonsterGladiator::State_Torso_RailgunAttack(const stateParms_t &parms)
{
	...
	case STAGE_WAIT:
		if (AnimDone(ANIMCHANNEL_TORSO, parms.blendFrames) || animator.CurrentAnim(ANIMCHANNEL_TORSO)->GetEndTime() < 0 // anim somehow cycled?!!!
			|| idStr::Icmp(animator.CurrentAnim(ANIMCHANNEL_TORSO)->AnimName(), "railgun_attack"))
		{ // anim changed
			return SRESULT_DONE;
		}
		return SRESULT_WAIT;
...
stateResult_t rvMonsterGladiator::State_Torso_TurnRight90(const stateParms_t &parms)
{
	...
	case STAGE_WAIT:
		if (move.fl.moving || AnimDone(ANIMCHANNEL_TORSO, 0) || animator.CurrentAnim(ANIMCHANNEL_TORSO)->GetEndTime() < 0 // anim somehow cycled?!!!
			|| idStr::Icmp(animator.CurrentAnim(ANIMCHANNEL_TORSO)->AnimName(), "turn_right"))
		{ // anim changed
			AnimTurn(0, true);
...
stateResult_t rvMonsterGladiator::State_Torso_TurnLeft90(const stateParms_t &parms)
{
	...
	case STAGE_WAIT:
		if (move.fl.moving || AnimDone(ANIMCHANNEL_TORSO, 0) || animator.CurrentAnim(ANIMCHANNEL_TORSO)->GetEndTime() < 0 // anim somehow cycled?!!!
			|| idStr::Icmp(animator.CurrentAnim(ANIMCHANNEL_TORSO)->AnimName(), "turn_left"))
		{ // anim changed
			AnimTurn(0, true);
			nextTurnTime = gameLocal.time + 250;
			return SRESULT_DONE;

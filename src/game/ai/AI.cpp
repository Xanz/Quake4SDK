...
void idAI::UpdateFocus(const idMat3 &orientationAxis)
{
	...
	else
	{
		...
		case AIFOCUS_LEADER:
			...
			break;
		case AIFOCUS_PLAYER:
			focusEnt = gameLocal.GetLocalPlayer();
			break;
		case AIFOCUS_TALK:
			focusEnt = talkTarget;
			break;
...
void idAI::OverrideFlag(aiFlagOverride_t flag, bool value)
{
	...
	switch (flag)
	{
	case AIFLAGOVERRIDE_DISABLEPAIN:
		oldValue = disablePain;
		break;
	case AIFLAGOVERRIDE_DAMAGE:
		oldValue = fl.takedamage;
		break;
	case AIFLAGOVERRIDE_NOTURN:
		oldValue = move.fl.noTurn;
		break;
	case AIFLAGOVERRIDE_NOGRAVITY:
		oldValue = move.fl.noGravity;
		break;
	default:
		return;
	}
	...
	case AIFLAGOVERRIDE_DAMAGE:
		...
		break;
	case AIFLAGOVERRIDE_NOTURN:
		move.fl.noTurn = value;
		break;
	case AIFLAGOVERRIDE_NOGRAVITY:
		move.fl.noGravity = value;
		break;
	}
}
...
void idAI::ClearEnemy(bool dead)
{
	...
	else if (!aifl.scripted && move.moveCommand == MOVE_TO_COVER && !move.fl.done && aasSensor->Reserved() && !IsBehindCover())
	{
	...
	if (canLook)
	{
		...
		if (animator.GetAnimFlags(animator.CurrentAnim(ANIMCHANNEL_TORSO)->AnimNum()).ai_look_head_only)
		{
			...
			if (newLookAng.Compare(lookAng))
			{
				...
				for (i = 0; i < lookJoints.Num(); i++)
				{
					if (lookJoints[i] != neckJoint && lookJoints[i] != headJoint)
					{
						// snap back now!
				...
				for (i = 0; i < lookJoints.Num(); i++)
				{
					if (lookJoints[i] != neckJoint && lookJoints[i] != headJoint)
					{
						...
						if (curJointMod)
						{
							...
							if (fabs(curJointAngleMod.pitch) < 1.0f && fabs(curJointAngleMod.yaw) < 1.0f && fabs(curJointAngleMod.roll) < 1.0f)
							{
								// snap back now!
								animator.ClearJoint(lookJoints[i]);
							}
...
void idAI::OnTouch(idEntity *other, trace_t *trace)
{
	...
	if (other->IsType(idActor::GetClassType()) && !other->fl.notarget && (ReactionTo(other) & ATTACK_ON_SIGHT) && (!enemy.ent || gameLocal.time - enemy.changeTime > 1000))
	{
		SetEnemy(other);
	}
...
void idAI::UpdateThreat(void)
{
	...
	case AITACTICAL_HIDE:
		combat.threatCurrent *= 0.5f;
		break;
	case AITACTICAL_MELEE:
		combat.threatCurrent *= 2.0f;
		break;
	}

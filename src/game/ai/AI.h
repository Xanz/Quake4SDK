...
{
	return (enemy.ent && combat.fl.seenEnemyDirectly && (enemy.lastVisibleTime && gameLocal.time - enemy.lastVisibleTime < (combat.maxLostVisTime * maxLostVisTimeScale)));
}

ID_INLINE bool idAI::LookAtCoverTall(void) const
{
	return (aasSensor->Look() && (aasSensor->Look()->flags & FEATURE_LOOK_OVER) && aasSensor->Look()->height > 40.0f);
}

ID_INLINE bool idAI::InLookAtCoverMode(void) const
{
	return (!IsBehindCover() && !aifl.action && move.fl.moving && (combat.fl.alert || combat.fl.aware) && aasSensor->Look() && combat.tacticalCurrent != AITACTICAL_MELEE && combat.tacticalCurrent != AITACTICAL_HIDE && !IsEnemyRecentlyVisible(0.2f));
}

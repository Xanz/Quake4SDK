...
bool rvMonsterStroggMarine::CheckAction_RangedAttack(rvAIAction *action, int animNum)
{
	...
	if (spawnArgs.GetBool("rangeAttackChanceInverse") && enemy.range - action->minRange > gameLocal.random.RandomFloat() * (action->maxRange - action->minRange))
	{
		// the father away you are, the more likely you are to not attack
		return false;
	}
	if (spawnArgs.GetBool("rangeAttackChance") && enemy.range - action->minRange < gameLocal.random.RandomFloat() * (action->maxRange - action->minRange))
	{
...
stateResult_t rvMonsterStroggMarine::State_Torso_SprayAttack(const stateParms_t &parms)
{
	...
	case STAGE_END:
		if (AnimDone(ANIMCHANNEL_TORSO, 0))
		{
			...
			if (sweepCount < 3 && (!gameLocal.random.RandomInt(2) || (spraySideRight && !curEnemyMovingRight) || (!spraySideRight && curEnemyMovingRight)))
			{

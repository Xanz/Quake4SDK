...
stateResult_t idAI::State_CombatRanged(const stateParms_t &parms)
{
	...
	case STAGE_MOVE:
		...
		if (!move.fl.noRangedInterrupt &&
			!aifl.simpleThink && enemy.fl.visible && gameLocal.time - enemy.lastVisibleChangeTime > 500 && enemy.range >= combat.attackRange[0] && enemy.range <= combat.attackRange[1] && (DistanceTo(move.moveDest) < DistanceTo(move.lastMoveOrigin)) && (!tether || tether->ValidateDestination(this, physicsObj.GetOrigin())) && aiManager.ValidateDestination(this, physicsObj.GetOrigin()))
		{
			StopMove(MOVE_STATUS_DONE);
			return SRESULT_STAGE(STAGE_ATTACK);
		}

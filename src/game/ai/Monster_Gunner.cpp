...
void rvMonsterGunner::OnStopMoving(aiMoveCommand_t oldMoveCommand)
{
	...
	if (GetEnemy())
	{
		...
		else if ((!actionNailgunAttack.timer.IsDone(actionTime) || !actionTimerRangedAttack.IsDone(actionTime)) && (!actionGrenadeAttack.timer.IsDone(actionTime) || !actionTimerSpecialAttack.IsDone(actionTime)))
		{ // no attack is ready
...
int rvMonsterGunner::FilterTactical(int availableTactical)
{
	...
	{ // keep moving!
		if ((!actionNailgunAttack.timer.IsDone(actionTime + 500) || !actionTimerRangedAttack.IsDone(actionTime + 500)) && (!actionGrenadeAttack.timer.IsDone(actionTime + 500) || !actionTimerSpecialAttack.IsDone(actionTime + 500)))
...
bool rvMonsterGunner::CheckActions(void)
{
	...
	if (!action)
	{
		...
		if (GetEnemy() && enemy.fl.visible && gameLocal.GetTime() - lastAttackTime > actionTimerRangedAttack.GetRate() + 1000)
		{
			...
			if (PerformAction(&actionSideStepLeft, (checkAction_t)&rvMonsterGunner::CheckAction_SideStepLeft, &actionTimerSideStep) || PerformAction(&actionSideStepRight, (checkAction_t)&rvMonsterGunner::CheckAction_SideStepRight, &actionTimerSideStep))
			{
				return true;
			}
		}

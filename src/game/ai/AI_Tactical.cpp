		...
		if (IsBehindCover())
		{
			...
			else
			{
				...
				else
				{
				 ...
				 && ((leader && leader->IsCrouching()) || combat.fl.crouchViewClear))
		{ // leader is crouching or we can crouch-look in this direction here
...
stateResult_t rvAITactical::State_Torso_SetPosture(const stateParms_t &parms)
{
	...
	case STAGE_INIT:
	{
		...
		if (postureCurrent < AIPOSTURE_STAND_COVER_LEFT || postureCurrent > AIPOSTURE_CROUCH_COVER_RIGHT || (postureIdeal != AIPOSTURE_STAND && postureIdeal != AIPOSTURE_RELAXED && postureIdeal != AIPOSTURE_CROUCH))
		{
		...
		if (postureCurrent >= AIPOSTURE_STAND_COVER_LEFT && postureCurrent <= AIPOSTURE_CROUCH_COVER_RIGHT && postureIdeal == AIPOSTURE_RELAXED)
		{
...
{
	if (postureCurrent == AIPOSTURE_CROUCH || postureCurrent == AIPOSTURE_CROUCH_COVER || postureCurrent == AIPOSTURE_CROUCH_COVER_LEFT || postureCurrent == AIPOSTURE_CROUCH_COVER_RIGHT)
	{
		return true;
	}

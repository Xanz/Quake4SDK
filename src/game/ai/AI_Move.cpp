...
float idAI::TravelDistance(const idVec3 &start, const idVec3 &end) const
{
	...
	if (DebugFilter(ai_debugMove))
	{ // Travel Distance, Fly Path & Walk Path
		if (move.moveType == MOVETYPE_FLY)
		{
...
void idAI::StopMove(moveStatus_t status)
{
	...
	if (move.moveType == MOVETYPE_FLY)
	{
		if (move.moveCommand == MOVE_TO_ENEMY || move.moveCommand == MOVE_TO_ATTACK)
		{
...
bool idAI::TurnTowardLeader(bool faceLeaderByDefault)
{
	...
	if (tr.fraction < 1.0f && (tr.fraction < 0.5f || !traceEnt || !traceEnt->IsType(idDoor::GetClassType())))
	{
		// wall there - NOTE: okay to look at doors
		if (faceLeaderByDefault // want to face leader by default
			|| leader->viewAxis[0].ToYaw() == move.ideal_yaw)
		{ // a wall must have moved in front of us?
			// face the leader
...
void TestTeammateCollisions(idAI *owner)
{
	...
	for (teammate = aiManager.GetAllyTeam((aiTeam_t)owner->team); teammate; teammate = teammate->teamNode.Next())
	{
		...
		if (teammateDirectionDot > 0.85f)
		{
			...
			if (owner->DebugFilter(ai_debugMove))
			{ // WHITE = Walk Teammate Near
...
void idAI::AnimMove(void)
{
	...
	if (DebugFilter(ai_debugMove))
	{ // AnimMove : GREEN / RED Bounds & Move Dest
...
struct rvObstacle
{
	...
	*/
	bool Update()
	{
		...
		if (origin.Dist2XY(physics->GetOrigin()) < 100.0f)
		{ // CDR_TODO: Scale This By Distance To Player, Other Obstacles Near...
			return true;
		}
...
void idAI::RVMasterMove(void)
{
	...
	if (seekMove)
	{
		...
		if (move.fl.obstacleInPath)
		{
			...
			{ // CDR_TODO:  -1 is an obtouse way to detect this
				move.fl.blocked = true;

...
int rvMonsterHarvester::GetDamageForLocation(int damage, int location)
{
		...
		for (int part = 0; part < PARTS_MAX; part++)
		{
			if (idStr::Icmp(dmgGroup, partLocation[part].c_str()) == 0)
			{
				if (partHealth[part] > 0)
				{
					...
					if (partHealth[part] <= 0)
					{
						if (animPrefix.Length() && (part == PART_LEG_FR || part == PART_LEG_FL || part == PART_LEG_BR || part == PART_LEG_BL))
	...
	if (health <= spawnArgs.GetInt("death_damage_threshold") && spawnArgs.GetInt("death_damage_threshold") > damage)

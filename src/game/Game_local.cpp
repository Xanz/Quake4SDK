...
#define MATCH(s) \
	(!kv->GetKey().Icmpn(s, strlen(s)))
		...
		else if (MATCH("gui_"))
		{
			...
			if (!idStr::Icmp(kv->GetKey(), "gui_noninteractive") || !idStr::Icmpn(kv->GetKey(), "gui_parm", 8) || !idStr::Icmp(kv->GetKey(), "gui_inventory"))
			{
	...
	for (reflect = hitscanDict.GetFloat("reflect", "0"); reflect >= 0; reflect--)
	{
		...
		while (1)
		{
			...
			if (!gameLocal.isClient)
			{
				...
				{
					if (actualHitEnt && actualHitEnt != ent && (tr.c.material->GetSurfaceFlags() & SURF_BOUNCE) && actualHitEnt->spawnArgs.GetBool("takeBounceDamage"))
					{ // bleh...

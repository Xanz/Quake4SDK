...
bool idProjectile::Collide(const trace_t &collision, const idVec3 &velocity, bool &hitTeleporter)
{
	...
	else
	{
		...
		if (bounce)
		{
			...
			if (actualHitEnt && actualHitEnt != ent && actualHitEnt->spawnArgs.GetBool("takeBounceDamage"))
			{ // bleh...
...
void idProjectile::DefaultDamageEffect(const trace_t &tr, const idVec3 &velocity, const char *damageDefName)
{
	...
	if ((!*spawnArgs.GetString("def_splash_damage") || spawnArgs.GetBool("bloodyImpactEffect")) && owner.GetEntity() && !ent->CanPlayImpactEffect(owner, ent))
	{
		return;
	}

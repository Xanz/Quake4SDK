	...
	else if (!idStr::Icmp(statname, "weapon"))
	{
		...
		for (pos = value; pos != NULL; pos = end)
		{
			...
			if (!gameLocal.world->spawnArgs.GetBool("no_Weapons") || (weaponName == "weapon_fists"))
			{
				...
				{
					if ((owner->GetUserInfo()->GetBool("ui_autoSwitch") || (gameLocal.isMultiplayer && gameLocal.mpGame.IsBuyingAllowedInTheCurrentGameMode())) && idealWeapon && !checkOnly)
					{
...
void idPlayer::UpdateWeapon(void)
{
	...
	else if (!hiddenWeapon)
	{ /* no pda yet || ( ( weapon_pda >= 0 ) && ( idealWeapon == weapon_pda ) ) ) { */
		flagCanFire = true;
		Weapon_Combat();
	}
...
// asalmon: auto-aim for Xenon, finds a best enemy and changes the crosshairs.
	...
	for (i = 0; i < listedClipModels; i++)
	{
		...
		if (!gameLocal.isMultiplayer && focusType == FOCUS_NONE && !g_crosshairCharInfoFar.GetBool())
		{
			if (focusLength < 512)
			{
...
// asalmon: D-pad events for in game guis on Xenon
	...
	case IMPULSE_40:
	{
		...
		// squirrel: Mode-agnostic buymenus
	case IMPULSE_100:
		AttemptToBuyItem("weapon_shotgun");
		break;
	case IMPULSE_101:
		AttemptToBuyItem("weapon_machinegun");
		break;
	case IMPULSE_102:
		AttemptToBuyItem("weapon_hyperblaster");
		break;
	case IMPULSE_103:
		AttemptToBuyItem("weapon_grenadelauncher");
		break;
	case IMPULSE_104:
		AttemptToBuyItem("weapon_nailgun");
		break;
	case IMPULSE_105:
		AttemptToBuyItem("weapon_rocketlauncher");
		break;
	case IMPULSE_106:
		AttemptToBuyItem("weapon_railgun");
		break;
	case IMPULSE_107:
		AttemptToBuyItem("weapon_lightninggun");
		break;
	case IMPULSE_108:
		break; // Unused
	case IMPULSE_109:
		AttemptToBuyItem("weapon_napalmgun");
		break;
	case IMPULSE_110: /* AttemptToBuyItem( "weapon_dmg" );*/
		break;
	case IMPULSE_111:
		break; // Unused
	case IMPULSE_112:
	...
	case IMPULSE_117:
		break; // Unused
	case IMPULSE_118:
		AttemptToBuyItem("item_armor_small");
		break;
	case IMPULSE_119:
		AttemptToBuyItem("item_armor_large");
		break;
	case IMPULSE_120:
		AttemptToBuyItem("ammorefill");
		break;
	case IMPULSE_121:
		break; // Unused
	case IMPULSE_122:
		break; // Unused
	case IMPULSE_123:
		AttemptToBuyItem("ammo_regen");
		break;
	case IMPULSE_124:
		AttemptToBuyItem("health_regen");
		break;
	case IMPULSE_125:
		AttemptToBuyItem("damage_boost");
		break;
	case IMPULSE_126:
		break; // Unused
	case IMPULSE_127:
...
void idPlayer::Killed(idEntity *inflictor, idEntity *attacker, int damage, const idVec3 &dir, int location)
{
	...
	bool noDrop = false;
	if (inflictor && inflictor->IsType(idTrigger_Hurt::GetClassType()) && inflictor->spawnArgs.GetBool("nodrop"))
	{
	...
	if (weapon)
	{						 // cnicholson: Fix for crash if player dies while in vehicle
								...
								const float damageScale, const int location, int *health, int *armor)
{
	...
	if (gameLocal.IsTeamGame() && !gameLocal.serverInfo.GetBool("si_teamDamage") && !damageDef->GetBool("noTeam") && player && player != this // you get self damage no matter what
		&& player->team == team)
	{
		damage = 0;
	}

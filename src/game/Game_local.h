...
#define SAFE_REMOVE(p)                   \
	if (p)                               \
	{                                    \
		(p)->PostEventMS(&EV_Remove, 0); \
		(p) = NULL;                      \
	}
// RAVEN END
...
public:
	...
	idEntityPtr<type> &operator=(idEntityPtr<type> &ent)
	{
		*this = ent.GetEntity();
		return *this;
	}
...
// jsinger: attempt to eliminate cross-DLL allocation issues
	...
	{
		idStr::Copynz(reason, "#str_107239" /* zinx - FIXME - not banned... */, sizeof(reason));
		return ALLOW_NO;
	};
	...
	{
		if (isRepeater || serverIsRepeater)
			Warning("Repeater does not work for single player.");
	};

...
public:
	...
	void OnEnter(void)
	{
		funcs.enter.CallFunc(NULL);
		fl.exitedVehicle = false;
	};
	void OnExit(void)
	{
		if (!fl.exitedVehicle)
		{
			fl.exitedVehicle = true;
			funcs.exit.CallFunc(NULL);
		}
	};
...
{
	return ((!vfl.flipEject) ? false : idMath::Fabs(idMath::AngleNormalize180(renderEntity.axis.ToAngles().roll)) > 60.f || idMath::Fabs(idMath::AngleNormalize180(renderEntity.axis.ToAngles().pitch) > 60.0f));
}

...
void idMover::Event_TeamBlocked(idEntity *blockedEntity, idEntity *blockingEntity)
{
	if (!blockingEntity->fl.takedamage)
	{
		...
		else if (blockingEntity->IsType(idMoveable::GetClassType()) || blockingEntity->IsType(idMoveableItem::GetClassType()) || (blockingEntity->IsType(idAFEntity_Base::GetClassType()) && !blockingEntity->IsType(idActor::GetClassType())))
		{
			// moveable
			blockingEntity->ProcessEvent(&EV_Remove);
			return;

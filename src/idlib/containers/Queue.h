...
public:
	...
	{
		assert(!empty());
		return mData[mOldest];
	}
	TYPE &last()
	{
		assert(!empty());
		return mData[mNewest - 1];
	}
	TYPE &operator[](int i)
	{
		assert(valid(i));
...
public:
	...
	{
		assert(valid(i));
		mData[i >> BITS_SHIFT] &= ~(1 << (i & BITS_AND));
	}
	...
	{
		assert(valid(i));
		return (mData[i >> BITS_SHIFT] & (1 << (i & BITS_AND))) != 0;
	}
};

...
namespace TextCompiler
{
	...
	inline void WriteValue(type const *const ptr, idFile *out, bool byteSwap = false)
	{
		if (out != NULL)
		{
			if (!byteSwap)
	...
	template <typename type>
	inline void WriteValue(type const val, idFile *out, bool byteSwap = false)
	{
		if (out != NULL)
		{
			if (!byteSwap)
	...
	template <typename type>
	inline type ReadValue(idFile *in)
	{
		type ret;
	...
	template <>
	inline void WriteValue(idStr const *const ptr, idFile *out, bool byteSwap)
	{
		if (out != NULL)
		{
	...
	template <>
	inline idStr ReadValue(idFile *in)
	{
		char c;
		idStr str;
	...
	template <typename type>
	inline void WriteArray(type const *const ptr, unsigned int count, idFile *out)
	{
	...
	template <typename type>
	inline type *ReadArray(idFile *in, unsigned int *count = NULL)
	{
		unsigned int len;
	...
	template <typename type>
	inline void WriteIdList(idList<type> const *const ptr, idFile *out)
	{
		WriteArray<type>(ptr->Ptr(), ptr->Num(), out);
	}

	template <typename type>
	inline idList<type> ReadIdList(idFile *in)
	{
		idList<type> ret;
		ret.Clear();
		unsigned int count;
